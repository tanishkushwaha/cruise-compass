...
const AddStationeryItemModal = ({ isOpen, onClose, setUpdate }: PropsType) => {
  ...
  const handleSubmit = () => {
    ...
    setLoading(true)
    axios.post('/api/stationeries', formData, {
      headers: {
      ...
      .then(res => {
        toast({
          title: 'Stationery item added successfully!',
          status: 'success',
          duration: 5000,
          isClosable: true,
          position: 'top-right'
      ...
      .catch(err => {
        toast({
          title: 'Error adding stationery item',
          status: 'error',
          duration: 5000,
          isClosable: true,
          position: 'top-right'
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleModalClose}>
      <ModalOverlay />
      <ModalContent>
        <ModalHeader>Add Stationery</ModalHeader>
        <ModalCloseButton />
        <ModalBody>
          <Stack direction='column' gap={5}>
            <FormInput label="Enter Stationery Name" name="name" value={inputData.name} onChange={handleChange} error={inputErrors.name} />
...
export default AddStationeryItemModal