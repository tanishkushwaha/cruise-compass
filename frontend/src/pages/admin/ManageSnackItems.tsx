...
import EditSnackModal from "../../components/EditSnackModal"


const ManageSnackItems = () => {
  const addSnackModal = useDisclosure()
  ...
  return (
    <Container h='90%' maxW='5xl' p='5rem'>
      ...
      <AddSnackModal isOpen={addSnackModal.isOpen} onClose={addSnackModal.onClose} setUpdate={setUpdate} />
      <FloatingAddButton onClick={addSnackModal.onOpen} />
    </Container >
  )
}
...
const ItemCard = ({ id, imgSrc, imgAlt = 'img', title, descr, price, setUpdate }: PropsType) => {
  ...
  const editSnackModal = useDisclosure()
  ...
  return (
    <Card maxW='100%'>
      ...
      <EditSnackModal isOpen={editSnackModal.isOpen} onClose={editSnackModal.onClose} setUpdate={setUpdate} id={id} name={title} description={descr} price={price} />
    </Card>
  )
}
